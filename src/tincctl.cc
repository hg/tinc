...
char *device = NULL;
char *iface = NULL;
debug_t debug_level = DEBUG_UNSET;
...
static bool ed25519_keygen(bool ask) {
	...
	pubkey = ecdsa_get_base64_public_key(key);
...
bool recvline(int fd, char *line, size_t len) {
	...
	while(!(newline = static_cast<char *>(memchr(buffer, '\n', blen)))) {
...
static void pcap(int fd, FILE *out, uint32_t snaplen) {
	...
	struct {
		...
		2, 4,
		0, 0,
		static_cast<uint32_t>(snaplen ? snaplen : sizeof(data)),
		1,
	};
...
static void control_log(int fd, FILE *out, int level) {
...
static int cmd_start(int argc, char *argv[]) {
	...
	int nargc = 0;
	char **nargv = static_cast<char **>(xzalloc((optind + argc) * sizeof(*nargv)));
...
static int cmd_log(int argc, char *argv[]) {
	...
	control_log(fd, stdout, argc > 1 ? atoi(argv[1]) : -1);
...
static int run_export(const char *name, FILE *out) {
	char filename[PATH_MAX];
...
static int cmd_export(int argc, char *argv[]) {
	...
	int result = run_export(name, stdout);
...
static int cmd_export_all(int argc, char *argv[]) {
	...
	while((ent = readdir(dir))) {
		...
		result |= run_export(ent->d_name, stdout);
	}
...
static void *readfile(FILE *in, size_t *len) {
	size_t count = 0;
	size_t bufsize = 4096;
	auto *buf = static_cast<char *>(xmalloc(bufsize));
	...
	while(!feof(in)) {
		...
		if(count >= bufsize) {
			bufsize *= 2;
			buf = static_cast<char *>(xrealloc(buf, bufsize));
		}
	}
...
static int cmd_sign(int argc, char *argv[]) {
	...
	size_t len;
	auto *data = static_cast<char *>(readfile(in, &len));
	...
	data = static_cast<char *>(xrealloc(data, len + trailer_len));
...
static int cmd_verify(int argc, char *argv[]) {
	...
	auto *newline = static_cast<char *>(memchr(data, '\n', len));
...
static char *complete_config(const char *text, int state) {
	...
	while(variables[i].name) {
		auto *dot = const_cast<char *>(strchr(text, '.'));
...
static int cmd_shell(int argc, char *argv[]) {
	...
	char *line = NULL;
	int maxargs = argc + 16;
	char **nargv = static_cast<char **>(xmalloc(maxargs * sizeof(*nargv)));
	...
	while(true) {
		...
		while(p && *p) {
			if(nargc >= maxargs) {
				maxargs *= 2;
				nargv = static_cast<char **>(xrealloc(nargv, maxargs * sizeof(*nargv)));
			}

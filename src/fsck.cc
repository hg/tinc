...
static void check_conffile(const char *nodename, bool server) {
	...
	auto *count = static_cast<int *>(alloca(countlen));
...
static bool test_rsa_keypair(rsa_t *rsa_priv, rsa_t *rsa_pub, const char *host_file) {
	...
	bool success = false;
	auto *plaintext = static_cast<uint8_t *>(xmalloc(len));
	auto *encrypted = static_cast<uint8_t *>(xzalloc(len));
	auto *decrypted = static_cast<uint8_t *>(xzalloc(len));

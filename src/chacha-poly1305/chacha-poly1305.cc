...
chacha_poly1305_ctx_t *chacha_poly1305_init(void) {
	chacha_poly1305_ctx_t *ctx = static_cast<chacha_poly1305_ctx_t *>(xzalloc(sizeof(*ctx)));
	return ctx;
}
...
bool chacha_poly1305_encrypt(chacha_poly1305_ctx_t *ctx, uint64_t seqnr, const void *indata, size_t inlen, void *voutdata, size_t *outlen) {
	...
	uint8_t *outdata = static_cast<uint8_t *>(voutdata);
	...
	chacha_encrypt_bytes(&ctx->main_ctx, static_cast<const uint8_t *>(indata), outdata, inlen);
...
bool chacha_poly1305_decrypt(chacha_poly1305_ctx_t *ctx, uint64_t seqnr, const void *vindata, size_t inlen, void *outdata, size_t *outlen) {
	...
	const uint8_t *indata = static_cast<const uint8_t *>(vindata);
	...
	chacha_encrypt_bytes(&ctx->main_ctx, indata, static_cast<uint8_t *>(outdata), inlen);

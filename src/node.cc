...
splay_tree_t node_tree = {
	...
	.release = (splay_action_t) free_node,
};
...
node_t *new_node(void) {
	node_t *n = static_cast<node_t *>(xzalloc(sizeof(*n)));

	if(replaywin) {
		n->late = static_cast<uint8_t *>(xzalloc(replaywin));
	}
...
node_t *lookup_node(char *name) {
	...
	return static_cast<node_t *>(splay_search(&node_tree, &n));
}

node_t *lookup_node_id(const node_id_t *id) {
	node_t n = {.id = *id};
	return static_cast<node_t *>(splay_search(&node_id_tree, &n));
}

node_t *lookup_node_udp(const sockaddr_t *sa) {
	node_t tmp = {.address = *sa};
	return static_cast<node_t *>(splay_search(&node_udp_tree, &tmp));
}

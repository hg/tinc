...
static void udp_probe_timeout_handler(void *data) {
	node_t *n = static_cast<node_t *>(data);
...
static void udp_probe_h(node_t *n, vpn_packet_t *packet, length_t len) {
	...
	if(udp_discovery) {
		timeval tv{udp_discovery_timeout, 0};
		timeout_del(&n->udp_ping_timeout);
		timeout_add(&n->udp_ping_timeout, &udp_probe_timeout_handler, n, &tv);
	}
...
static bool receive_udppacket(node_t *n, vpn_packet_t *inpkt) {
	...
	if(n->incompression != COMPRESS_NONE) {
		...
		if(!(outpkt->len = uncompress_packet(DATA(outpkt), DATA(inpkt), inpkt->len,
		                                     static_cast<compression_level_t>(n->incompression)))) {
...
static void send_sptps_packet(node_t *n, vpn_packet_t *origpkt) {
	...
	if(n->outcompression != COMPRESS_NONE) {
		outpkt.offset = 0;
		length_t len = compress_packet(DATA(&outpkt) + offset, DATA(origpkt) + offset, origpkt->len - offset,
		                               static_cast<compression_level_t>(n->outcompression));
...
static void send_udppacket(node_t *n, vpn_packet_t *origpkt) {
	...
	if(n->outcompression != COMPRESS_NONE) {
		...
		if(!(outpkt->len = compress_packet(DATA(outpkt), DATA(inpkt), inpkt->len,
		                                   static_cast<compression_level_t>(n->outcompression)))) {
...
bool send_sptps_data(node_t *to, node_t *from, int type, const void *data, size_t len) {
	...
	if(type == SPTPS_HANDSHAKE || tcponly || (!direct && !relay_supported) || (type != PKT_PROBE && origlen > relay->minmtu)) {
		if(type != SPTPS_HANDSHAKE && (to->nexthop->connection->options >> 24) >= 7) {
			...
			uint8_t *buf = static_cast<uint8_t *>(alloca(buflen));
			uint8_t *buf_ptr = buf;
		...
		char *buf = static_cast<char *>(alloca(B64_SIZE(len)));
	...
	char *buf = static_cast<char *>(alloca(len + overhead));
	char *buf_ptr = buf;
...
bool receive_sptps_record(void *handle, uint8_t type, const void *data, uint16_t len) {
	node_t *from = static_cast<node_t *>(handle);
	...
	if(type & PKT_COMPRESSED) {
		length_t ulen = uncompress_packet(DATA(&inpkt) + offset, (const uint8_t *)data, len,
		                                  static_cast<compression_level_t>(from->incompression));
...
void broadcast_packet(const node_t *from, vpn_packet_t *packet) {
	...
	case BMODE_MST:
		for list_each(connection_t, c, &connection_list)
			if(c->edge && c->status.bits.mst && c != from->nexthop->connection) {
...
void handle_incoming_vpn_data(void *data, int flags) {
	(void)data;
	(void)flags;
	listen_socket_t *ls = static_cast<listen_socket_t *>(data);

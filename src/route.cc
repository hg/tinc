...
static bool do_decrement_ttl(node_t *source, vpn_packet_t *packet) {
	...
	case ETH_P_IP: {
		...
		uint16_t next = DATA(packet)[ethlen + 8] << 8 | DATA(packet)[ethlen + 9];

		uint32_t checksum = DATA(packet)[ethlen + 10] << 8 | DATA(packet)[ethlen + 11];
		checksum += old + (~next & 0xFFFF);
...
static void age_subnets(void *data) {
	...
	if(left) {
		...
		timeout_set(&age_subnets_timeout, &tv);
	}
}
...
static void learn_mac(mac_t *address) {
	...
	if(!subnet) {
		...
		timeval tv{10, jitter()};
		timeout_add(&age_subnets_timeout, age_subnets, NULL, &tv);
	} else {
		if(subnet->expires) {
			subnet->expires = now.tv_sec + macexpire;
		}

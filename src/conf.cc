...
list_t cmdline_conf = {
	...
	.tail = NULL,
	.count = 0,
	.release = (list_action_t)free_config,
};
...
splay_tree_t config_tree = {
	...
	.release = (splay_action_t) free_config,
};
...
void init_configuration(splay_tree_t *tree) {
	...
	tree->release = (splay_action_t) free_config;
}
...
config_t *new_config(void) {
	return static_cast<config_t *>(xzalloc(sizeof(config_t)));
}
...
config_t *lookup_config(splay_tree_t *config_tree, const char *variable) {
	...
	auto *found = static_cast<config_t *>(splay_search_closest_greater(config_tree, &cfg));
...
config_t *lookup_config_next(splay_tree_t *config_tree, const config_t *cfg) {
	...
	if(node) {
		if(node->next) {
			found = static_cast<config_t *>(node->next->data);
...
void read_config_options(splay_tree_t *config_tree, const char *prefix) {
	...
	for(const list_node_t *node = cmdline_conf.tail; node; node = node->prev) {
		auto *cfg = static_cast<const config_t *>(node->data);
		config_t *new_conf;
		...
		if(prefix) {
			new_conf->variable = xstrdup(cfg->variable + prefix_len + 1);
		} else {
			new_conf->variable = xstrdup(cfg->variable);
		}

		new_conf->value = xstrdup(cfg->value);
		new_conf->file = NULL;
		new_conf->line = cfg->line;

		config_add(config_tree, new_conf);
	}
}

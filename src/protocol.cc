...
static splay_tree_t past_request_tree = {
	...
	.release = (splay_action_t) free_past_request,
};
...
void forward_request(connection_t *from, const char *request) {
	...
	char *tmp = static_cast<char *>(alloca(tmplen));
...
static void age_past_requests(void *data) {
	...
	if(left) {
		...
		timeout_set(&past_request_timeout, &tv);
	}
}

bool seen_request(const char *request) {
	past_request_t *new_req, p {};
	...
	if(splay_search(&past_request_tree, &p)) {
		...
		new_req = static_cast<past_request_t *>(xmalloc(sizeof(*new_req)));
		new_req->request = xstrdup(request);
		new_req->firstseen = now.tv_sec;
		splay_insert(&past_request_tree, new_req);


		timeval tv{10, jitter()};
		timeout_add(&past_request_timeout, age_past_requests, NULL, &tv);

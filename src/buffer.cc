...
char *buffer_prepare(buffer_t *buffer, uint32_t size) {
	if(!buffer->data) {
		buffer->maxlen = size;
		buffer->data = static_cast<char *>(xmalloc(size));
	} else {
		...
		if(buffer->len + size > buffer->maxlen) {
			...
			buffer->data = static_cast<char *>(xrealloc(buffer->data, buffer->maxlen));
		}
	}
...
char *buffer_readline(buffer_t *buffer) {
	auto *newline = static_cast<char *>(memchr(buffer->data + buffer->offset, '\n', buffer->len - buffer->offset));

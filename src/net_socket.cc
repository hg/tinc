...
list_t outgoing_list = {
	...
	.tail = NULL,
	.count = 0,
	.release = (list_action_t)free_outgoing,
};
...
static void retry_outgoing_handler(void *data) {
	auto *conn = static_cast<outgoing_t *>(data);
	setup_outgoing_connection(conn, true);
}
...
void retry_outgoing(outgoing_t *outgoing) {
	...
	timeval tv{outgoing->timeout, jitter()};
	timeout_add(&outgoing->ev, retry_outgoing_handler, outgoing, &tv);
...
void finish_connecting(connection_t *c) {
	...
	c->status.bits.connecting = false;
...
static void handle_meta_io(void *data, int flags) {
	connection_t *c = static_cast<connection_t *>(data);

	if(c->status.bits.connecting) {
		/*
...
void handle_new_meta_connection(void *data, int flags) {
	(void)flags;
	listen_socket_t *l = static_cast<listen_socket_t *>(data);
	connection_t *c;
	sockaddr_t sa;
	int fd;
...
void try_outgoing_connections(void) {
	...
	for(config_t *cfg = lookup_config(&config_tree, "ConnectTo"); cfg; cfg = lookup_config_next(&config_tree, cfg)) {
		...
		if(!found) {
			outgoing_t *outgoing = static_cast<outgoing_t *>(xzalloc(sizeof(*outgoing)));

...
static bool send_record_priv_datagram(sptps_t *s, uint8_t type, const void *data, uint16_t len) {
	uint8_t *buffer = static_cast<uint8_t *>(alloca(len + 21UL));
...
static bool send_record_priv(sptps_t *s, uint8_t type, const void *data, uint16_t len) {
	...
	uint8_t *buffer = static_cast<uint8_t *>(alloca(len + 19UL));
...
static bool send_kex(sptps_t *s) {
	...
	s->mykex = static_cast<uint8_t *>(realloc(s->mykex, 1 + 32 + keylen));
...
static bool send_sig(sptps_t *s) {
	...
	uint8_t *msg = static_cast<uint8_t *>(alloca(msglen));
	uint8_t *sig = static_cast<uint8_t *>(alloca(siglen));
...
static bool generate_key_material(sptps_t *s, const uint8_t *shared, size_t len) {
	...
	s->key = static_cast<uint8_t *>(realloc(s->key, keylen));
	...
	uint8_t *seed = static_cast<uint8_t *>(alloca(s->labellen + 64 + 13));
...
static bool receive_kex(sptps_t *s, const uint8_t *data, uint16_t len) {
	...
	s->hiskex = static_cast<uint8_t *>(realloc(s->hiskex, len));
...
bool sptps_verify_datagram(sptps_t *s, const void *vdata, size_t len) {
	...
	uint8_t *buffer = static_cast<uint8_t *>(alloca(len));
	size_t outlen;
...
size_t sptps_receive_data(sptps_t *s, const void *vdata, size_t len) {
	...
	if(s->buflen < 2) {
		...
		s->inbuf = static_cast<uint8_t *>(realloc(s->inbuf, s->reclen + 19UL));
...
bool sptps_start(sptps_t *s, void *handle, bool initiator, bool datagram, ecdsa_t *mykey, ecdsa_t *hiskey, const void *label, size_t labellen, send_data_t send_data, receive_record_t receive_record) {
	...
	if(s->replaywin) {
		s->late = static_cast<uint8_t *>(malloc(s->replaywin));
	...
	s->label = static_cast<uint8_t *>(malloc(labellen));
	...
	if(!datagram) {
		s->inbuf = static_cast<uint8_t *>(malloc(7));

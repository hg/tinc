...
ecdsa_t *ecdsa_set_base64_public_key(const char *p) {
	...
	ecdsa_t *ecdsa = static_cast<ecdsa_t *>(xzalloc(sizeof(*ecdsa)));
	len = b64decode_tinc(p, ecdsa->pub, len);
...
char *ecdsa_get_base64_public_key(ecdsa_t *ecdsa) {
	char *base64 = static_cast<char *>(xmalloc(44));
	b64encode_tinc(ecdsa->pub, base64, sizeof(ecdsa->pub));
...
ecdsa_t *ecdsa_read_pem_public_key(FILE *fp) {
	...
	if(read_pem(fp, "ED25519 PUBLIC KEY", ecdsa->pub, sizeof(ecdsa->pub))) {
		return ecdsa;
	}
...
ecdsa_t *ecdsa_read_pem_private_key(FILE *fp) {
	ecdsa_t *ecdsa = static_cast<ecdsa_t *>(xmalloc(sizeof(*ecdsa)));

	if(read_pem(fp, "ED25519 PRIVATE KEY", ecdsa->priv, sizeof(*ecdsa))) {
		return ecdsa;
	}
...
bool ecdsa_sign(ecdsa_t *ecdsa, const void *in, size_t len, void *sig) {
	ed25519_sign(static_cast<unsigned char *>(sig), static_cast<const unsigned char *>(in), len, ecdsa->pub, ecdsa->priv);
	return true;
}

bool ecdsa_verify(ecdsa_t *ecdsa, const void *in, size_t len, const void *sig) {
	return ed25519_verify(static_cast<const unsigned char *>(sig), static_cast<const unsigned char *>(in), len, ecdsa->pub);
}

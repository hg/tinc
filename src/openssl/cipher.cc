...
bool cipher_set_key(cipher_t *cipher, void *key, bool encrypt) {
	...
	return cipher_init_ctx(cipher, encrypt, static_cast<const unsigned char *>(key), iv);
}
...
static bool cipher_encrypt_decrypt(cipher_t *cipher, const void *indata, size_t inlen, void *outdata, size_t *outlen, bool oneshot,
	...
	if(oneshot) {
		...
		if(init(cipher->ctx, NULL, NULL, NULL, NULL)
		                && update(cipher->ctx, (unsigned char *)outdata, &len,
		                          static_cast<const unsigned char *>(indata), (int)inlen)
		...
		if(update(cipher->ctx, static_cast<unsigned char *>(outdata), &len, static_cast<const unsigned char *>(indata), (int)inlen)) {
			if(outlen) {
				*outlen = len;
			}

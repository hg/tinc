...
bool digest_create(digest_t *digest, const void *indata, size_t inlen, void *outdata) {
	...
	unsigned char *tmpdata = static_cast<unsigned char *>(alloca(len));
	...
	if(digest->hmac_ctx) {
		...
		ok = HMAC_Init_ex(digest->hmac_ctx, NULL, 0, NULL, NULL)
		     && HMAC_Update(digest->hmac_ctx, static_cast<const unsigned char *>(indata), inlen)
		     && HMAC_Final(digest->hmac_ctx, tmpdata, NULL);
#else
		EVP_MAC_CTX *mac_ctx = EVP_MAC_CTX_dup(digest->hmac_ctx);

		ok = mac_ctx
		     && EVP_MAC_update(mac_ctx, static_cast<const unsigned char *>(indata), inlen)
...
bool digest_verify(digest_t *digest, const void *indata, size_t inlen, const void *cmpdata) {
	...
	unsigned char *outdata = static_cast<unsigned char *>(alloca(len));

...
#else
	selection = bn_d ? EVP_PKEY_KEYPAIR : EVP_PKEY_PUBLIC_KEY;
...
static bool rsa_encrypt_decrypt(rsa_t *rsa, const void *in, size_t len, void *out,
	...
	if(ctx) {
		...
		bool ok = init(ctx) > 0
		          ...
		          && process(ctx, static_cast<unsigned char *>(out), &outlen, static_cast<const unsigned char *>(in), len) > 0
		          && outlen == len;
...
bool rsa_public_encrypt(rsa_t *rsa, const void *in, size_t len, void *out) {
	...
	if((size_t)RSA_public_encrypt((int) len, static_cast<const unsigned char *>(in), static_cast<unsigned char *>(out), rsa, RSA_NO_PADDING) == len) {
#else
...
bool rsa_private_decrypt(rsa_t *rsa, const void *in, size_t len, void *out) {
	...
	if((size_t)RSA_private_decrypt((int) len, static_cast<const unsigned char *>(in), static_cast<unsigned char *>(out), rsa, RSA_NO_PADDING) == len) {
#else

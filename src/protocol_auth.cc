...
static bool send_proxyrequest(connection_t *c) {
	...
	case PROXY_SOCKS4: {
		...
		uint8_t *s4req = static_cast<uint8_t *>(alloca(s4reqlen));
		s4req[0] = 4;
		s4req[1] = 1;
	...
	case PROXY_SOCKS5: {
		...
		uint8_t *s5req = static_cast<uint8_t *>(alloca(len));
...
static bool receive_invitation_sptps(void *handle, uint8_t type, const void *data, uint16_t len) {
	connection_t *c = static_cast<connection_t *>(handle);

	if(type == 128) {
		return true;
	}

	if(type == 1 && c->status.bits.invitation_used) {
		return finalize_invitation(c, static_cast<const char *>(data), len);
	}

	if(type != 0 || len != 18 || c->status.bits.invitation_used) {
		return false;
	}
	...
	char *hashbuf = static_cast<char *>(alloca(hashbuflen));
	char cookie[64];
	...
	c->status.bits.invitation_used = true;
...
bool id_h(connection_t *c, const char *request) {
	...
	if(c->protocol_minor >= 2) {
		...
		char *label = static_cast<char *>(alloca(labellen));
...
bool send_metakey(connection_t *c) {
	...
	*/
	...
	char *key = static_cast<char *>(alloca(len));
	char *enckey = static_cast<char *>(alloca(len));
	char *hexkey = static_cast<char *>(alloca(2 * len + 1));
...
bool send_challenge(connection_t *c) {
	...
	c->hischallenge = static_cast<uint8_t *>(xrealloc(c->hischallenge, len));
...
bool challenge_h(connection_t *c, const char *request) {
	...
	c->mychallenge = static_cast<uint8_t *>(xrealloc(c->mychallenge, len));
...
bool send_chal_reply(connection_t *c) {
	...
	char *digest = static_cast<char *>(alloca(digestlen * 2 + 1));

...
#endif
#endif
	char *var = static_cast<char *>(alloca(len + 1));
	strncpy(var, p, len);
	var[len] = 0;
#ifdef HAVE_UNSETENV
	unsetenv(var);
...
int environment_add(environment_t *env, const char *format, ...) {
	if(env->n >= env->size) {
		...
		env->entries = static_cast<char **>(xrealloc(env->entries, env->size * sizeof(*env->entries)));
	}
...
void environment_init(environment_t *env) {
	...
	env->entries = static_cast<char **>(xzalloc(env->size * sizeof(*env->entries)));
...
bool execute_script(const char *name, environment_t *env) {
	...
	if(!*scriptextension) {
		...
		char *fullname = static_cast<char *>(alloca(fullnamelen));

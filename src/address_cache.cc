...
static struct addrinfo *get_known_addresses(node_t *n) {
	...
	for splay_each(edge_t, e, &n->edge_tree) {
		...
		oai = ai;
		ai = static_cast<addrinfo *>(xzalloc(sizeof(*ai)));
		ai->ai_family = e->reverse->address.sa.sa_family;
		ai->ai_socktype = SOCK_STREAM;
		ai->ai_protocol = IPPROTO_TCP;
		ai->ai_addrlen = SALEN(e->reverse->address.sa);
		ai->ai_addr = static_cast<sockaddr *>(xmalloc(ai->ai_addrlen));
...
address_cache_t *open_address_cache(node_t *node) {
	address_cache_t *cache = static_cast<address_cache_t *>(xmalloc(sizeof(*cache)));
	cache->node = node;

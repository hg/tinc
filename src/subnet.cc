...
splay_tree_t subnet_tree = {
	...
	.release = (splay_action_t) free_subnet,
};
...
subnet_t *new_subnet(void) {
	return static_cast<subnet_t *>(xzalloc(sizeof(subnet_t)));
}
...
subnet_t *lookup_subnet(node_t *owner, const subnet_t *subnet) {
	return static_cast<subnet_t *>(splay_search(&owner->subnet_tree, subnet));
}
...
subnet_t *lookup_subnet_mac(const node_t *owner, const mac_t *address) {
	...
	if((r = static_cast<subnet_t *>(hash_search(mac_t, &mac_cache, address)))) {
		return r;
	}
...
subnet_t *lookup_subnet_ipv4(const ipv4_t *address) {
	...
	if((r = static_cast<subnet_t *>(hash_search(ipv4_t, &ipv4_cache, address)))) {
		return r;
	}
...
subnet_t *lookup_subnet_ipv6(const ipv6_t *address) {
	...
	if((r = static_cast<subnet_t *>(hash_search(ipv6_t, &ipv6_cache, address)))) {
		return r;
	}

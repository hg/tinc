...
splay_tree_t *splay_alloc_tree(splay_compare_t compare, splay_action_t release) {
	splay_tree_t *tree;

	tree = static_cast<splay_tree_t *>(xzalloc(sizeof(splay_tree_t)));
...
splay_node_t *splay_alloc_node(void) {
	return static_cast<splay_node_t *>(xzalloc(sizeof(splay_node_t)));
}

void splay_free_node(splay_tree_t *tree, splay_node_t *node) {
	if(node->data && tree->release) {
		tree->release(node->data);
	}
...
splay_node_t *splay_insert(splay_tree_t *tree, void *data) {
	splay_node_t *closest, *next;
	int result;

	if(!tree->root) {
		next = splay_alloc_node();
		next->data = data;
		splay_insert_top(tree, next);
	} else {
		...
		if(result < 0) {
			splay_insert_before(tree, closest, next);
		} else {
			splay_insert_after(tree, closest, next);
		}
	}

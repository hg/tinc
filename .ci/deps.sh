...
deps_linux_debian() {
  ...
  apt-get install -y \
    git binutils meson pkgconf gcc diffutils sudo texinfo texlive netcat-openbsd procps socat \
    zlib1g-dev:"$HOST" \
    libssl-dev:"$HOST" \
    liblzo2-dev:"$HOST" \
    liblz4-dev:"$HOST" \
...
deps_linux_rhel() {
  ...
  yum install -y \
    git binutils meson pkgconf gcc diffutils sudo texinfo-tex netcat procps systemd \

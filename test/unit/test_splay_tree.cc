...
static int test_teardown(void **state) {
	splay_delete_tree(static_cast<splay_tree_t *>(*state));
	return 0;
}
...
static void multiply_id_splay_node(splay_node_t *node) {
	node_t *t = static_cast<node_t *>(node->data);
	t->id *= 2;
	++multiply_splay_node_calls;
}

static void test_splay_foreach(void **state) {
	splay_tree_t *tree = static_cast<splay_tree_t *>(*state);
...
static void test_splay_basic_ops(void **state) {
	...
	node_t *found_one = static_cast<node_t *>(splay_search(tree, node));
	...
	found_one = static_cast<node_t *>(splay_search(tree, node));

...
from testlib import Test, Tinc, log, check, cmd
...
def init(t: Test) -> T.Tuple[Tinc, Tinc]:
    ...
    log.info("initialize node %s", foo)
    foo.cmd("init", foo.name)
    check.equals(foo.name, cmd.get(foo, "Name"))

    return foo, bar


with Test("test case sensitivity") as t:
    foo, _ = init(t)
...
with Test("single Mode variable is permitted") as t:
    foo, _ = init(t)
    foo.cmd("add", "Mode", "switch")
    foo.cmd("add", "Mode", "hub")
    check.equals("hub", cmd.get(foo, "Mode"))

with Test("test addition/deletion of multivalued variables") as t:
    foo, _ = init(t)
    for i in range(1, 4):
        sub = f"{i}.{i}.{i}.{i}"
...
with Test("cannot get/set server variables using node.variable syntax") as t:
    foo, _ = init(t)
    name, _ = foo.cmd("get", f"{foo.name}.Name", code=1)
    check.false(name)
    foo.cmd("set", f"{foo.name}.Name", "fake", code=1)

with Test("get/set host variables for other nodes") as t:
    foo, bar = init(t)
...
with Test("cannot get/set variables for nodes with invalid names") as t:
    foo, _ = init(t)

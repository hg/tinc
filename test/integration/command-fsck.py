...
node = Tinc()
stdout, _ = node.tincd('--version').communicate()
...
class Context:
    ...
    def expect_msg(self, msg: str, force: bool = False, code: int = 1, present: bool = True) -> None:
        ...
        if force:
            args.insert(0, '--force')
        stdout, stderr = self.t.cmd(*args, code=code)
...
for suffix in ('up', 'down'):
    ...
    with open(ctx.t.sub(f'fake-{suffix}'), 'w') as f:
        f.write('')
...
def test_private_key_var(var: str, file: str):
    ...
    renamed = os.path.realpath(ctx.t.sub('renamed_key'))
    os.rename(src=ctx.t.sub(file), dst=renamed)
    with open(ctx.conf, 'a') as f:
        f.write(f'\n{var} = {renamed}\n')
...
def test_ec_public_key_file_var(ctx: Context, *paths: str):
    ec_pubkey = os.path.realpath(ctx.t.sub('ec_pubkey'))
...
test_ec_public_key_file_var(ctx, 'hosts', ctx.t.name)
...
def test_rsa_public_key_file_var(ctx: Context, *paths: str):
    ...
    rsa_pub = os.path.realpath(ctx.t.sub('rsa_pubkey'))
...
test_rsa_public_key_file_var(ctx, 'hosts', ctx.t.name)
